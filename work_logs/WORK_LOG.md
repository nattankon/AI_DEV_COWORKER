...
## 2026-08-19 - Keep Cowork and Code processing status visible through streaming

- Diagnosed why Cowork appeared to lose its live status and elapsed timer while an agent task was still running: every intermediate `message.assistant` streaming delta was treated as a completed turn by both the reducer and the app-level busy-session registry. The backend status telemetry was already truthful; the renderer ended the visual processing state too early.
- Cowork and Code now preserve their busy/transient status through intermediate assistant streams and clear it only on final output, failure, cancellation/idle, or session completion. Chat keeps its existing behavior: research status disappears when answer streaming begins and the streaming answer takes over.
- TDD evidence: added an app-level Cowork regression that starts a request, receives an intermediate assistant stream, verifies the elapsed status remains visible, then receives the final assistant event and verifies the status clears. The test failed before the renderer fix and passed afterward.
- Release preparation: bumped the desktop package to `0.1.28` and built `AI-Dev-Co-worker-Setup-0.1.28.exe`, its blockmap, and a matching `latest.yml`. Both the startup installer gate and the post-window top-right Update path remain covered by the updater contract.
...
## 2026-08-19 - Add real three-level approval profiles without widening workspace scope

- Confirmed the project boundary before changing permissions: `WorkspaceTools._resolve()` rejects traversal and absolute paths outside the selected root even when its approval callback returns allow, Electron accepts workspace paths only after user folder selection, and Secret Guard continues to block or hide secret-classified paths inside the project.
- Replaced the renderer's legacy two-state `Ask before write` / `Auto-approve` toggle with a three-profile permission menu: `Manual control`, `Approvals only`, and `Full access`. The menu states the selected project boundary and that Workspace path validation plus Secret Guard remain enforced in every mode.
- Added `permission_policy.py` as the backend source of truth. `manual` always prompts; `trusted` auto-approves routine project writes and allowlisted verification but prompts for every external MCP write, Python code, restore/destructive operations, and unknown actions; `full` auto-approves only known supported side effects. Unknown approval kinds remain fail-closed.
- Wired the profile through React, the bridge adapter, Electron preload/main IPC, and `ipc_sidecar.py`. The selected profile persists in local storage, and installations carrying the old boolean setting migrate to `manual` or `full`. The legacy IPC command remains compatible. Restore operations are now classified as destructive rather than routine writes.
- TDD evidence: new policy, sidecar, bridge, Electron contract, menu, and CoworkApp tests failed before implementation and passed afterward. Existing workspace/Secret Guard tests verified that auto-approved actions cannot escape the project root or reveal protected files.
- Verification: focused permission UI/bridge tests passed `68/68`; workspace, Secret Guard, and approval-policy tests passed `34/34`; full backend suite passed `425/425`; full frontend suite passed `27/27` files and `194/194` tests; `npm run build` passed; `git diff --check` reported no whitespace errors beyond Windows line-ending notices. Browser preview verification at `1280x720` confirmed the menu stayed inside the viewport, showed all three descriptions and the boundary notice, and switched to `Approvals only` correctly.
- Release decision: source-only change held for the next planned update batch. No version bump, installer build, GitHub Release, or updater publication was created.
- Skills used: `writing-plans`, `systematic-debugging`, `test-driven-development`, `webapp-testing`, `browser:control-in-app-browser`, and `verification-before-completion`.

## 2026-08-21 - Context-window conversation continuity and Anthropic provider

- Replaced effort-counted short Chat history with a token-aware conversation planner driven by the selected model's catalog context window. Recent complete turns remain verbatim at Low, Medium, and High effort; only history that cannot fit is compacted into a bounded summary, with prompt/output headroom reserved and image data URLs charged as attachment markers instead of raw base64 text.
- Increased durable Chat-history retention to 1,200 messages by default through `COWORK_CHAT_HISTORY_RETENTION`. Added context diagnostics for the resolved window, history budget, sent/compacted message counts, and summary use. The summary cache avoids repeating the same compaction call during one sidecar lifetime.
- Added Anthropic as a first-class provider with Claude Opus 4.1, Claude Sonnet 4, and Claude 3.5 Haiku catalog entries. The native Messages API adapter supports system blocks, text, base64 image inputs, shared-loop tool schemas, tool-use responses, and tool-result continuations without forcing Anthropic through an OpenAI-compatible endpoint.
- Updated Settings > Providers with an Anthropic / Claude key field and provider-independent reveal/hide controls. Keys continue to use the existing local per-provider store and survive normal desktop updates; no key value is rendered after refresh.
- In-session review found and resolved two blocking correctness issues before completion: an oversized newest history turn could overflow the planner budget, and shared-loop assistant tool calls used a nested `function` shape that the first Anthropic translator did not read. Regression tests now cover both paths. A dedicated reviewer/subagent capability was unavailable, so review was completed in-session.
- Verification: full backend suite passed `433/433`; full frontend suite passed `27/27` files and `195/195` tests; `npm run build` passed; `git diff --check` reported no whitespace errors beyond existing Windows line-ending notices. Browser preview verification confirmed the Anthropic provider row, masked key input, reveal control, save control, helper text, and scroll layout are visible and coherent in the Settings dialog.
- Release decision: source-only change held for the next planned update batch. No version bump, installer build, GitHub Release, or updater publication was created.
- Skills used: `writing-plans`, `systematic-debugging`, `test-driven-development`, `requesting-code-review` (in-session fallback), `webapp-testing`, `browser:control-in-app-browser`, and `verification-before-completion`.

## 2026-08-21 - Add a generic Custom Anthropic-compatible provider

- Added a provider profile that stores only a normalized HTTP(S) API root and imported model IDs in `custom_anthropic_provider.json`. API keys remain in the existing per-provider credential store under a separate `anthropic_compatible` slot; profile files, IPC payloads, model metadata, and UI status never expose the key.
- Extended the Anthropic Messages API adapter with a configurable Base URL and the `anthropic-compatible:` model prefix. Official Anthropic remains an independent provider with its original endpoint. Imported custom models participate in explicit model selection, tool research, vision attachment handling, and context-window planning; Auto routing does not opt users into a custom endpoint.
- Added Settings > Providers controls for Base URL, masked/revealable key entry, Save, and Test & import models. The UI clearly states that a third-party endpoint receives the key and requests, shows imported model count and bounded success/error feedback, and never reloads the saved key into the renderer.
- Live diagnostic used the provided credential transiently without persisting or logging it. `GET /v1/models` authenticated and returned seven model IDs; generation through both supported protocol shapes returned HTTP 429 because the third-party provider reported that all backing accounts were exhausted. The app therefore supports the configuration, while current response availability still depends on the provider.
- TDD and verification: new provider/profile/adapter/sidecar/UI tests were added. Full backend suite passed `440/440`; full frontend suite passed `27/27` files and `196/196` tests; `npm run build` passed with the existing non-blocking chunk-size advisory. Browser preview at 1280x720 confirmed the custom card, scrolling, labels, controls, no horizontal overflow, and no console warnings/errors.
- Release decision: source-only change; no version bump, installer build, GitHub Release, or updater publication was created.
- Skills used: `writing-plans`, `systematic-debugging`, `test-driven-development`, `requesting-code-review` (in-session fallback), `webapp-testing`, `browser:control-in-app-browser`, and `verification-before-completion`.

## 2026-08-21 - Prepare desktop update v0.1.29

- Batched the unreleased three-level permission profiles, context-window conversation continuity, official Anthropic provider, and Custom Anthropic-compatible provider into desktop version `0.1.29` without changing either updater path.
- Fresh release verification passed: backend `440/440`; frontend `27/27` files and `196/196` tests; the production Vite build passed with only the existing non-blocking chunk-size advisory.
...
## 2026-08-21 - Add the isolated Web Chat browser surface (Phase 1)

- Documented the phased Web Chat bridge in `WEB_CHAT_BRIDGE_PLAN.md`. Web Chat is an Electron browser surface rather than a fourth native runtime, so Chat/Cowork/Code keep separate native sessions, tools, memory, and permission semantics.
- Added a sandboxed Electron `WebContentsView` for `https://chatgpt.com/` using `persist:web-chat`. The remote page has no preload bridge, no Node integration, and no app IPC access. The app does not read cookies or extract/reuse OAuth tokens. HTTPS/HTTP popup windows retain the same isolated partition for normal sign-in flows.
- Added bounded IPC for view state, bounds, visibility, back, forward, reload, home, and open-in-browser. Arbitrary renderer-supplied navigation URLs are not accepted.
- Added a `Web` mode control and browser toolbar. Web Chat does not change the selected native mode or create a native conversation. Native composer, search, and session history are hidden while the website surface is active, then restored when the user returns to Chat/Cowork/Code.
- TDD evidence: the new Web Chat surface tests initially failed because the modules did not exist, and the sidebar-isolation test initially failed because native history was still rendered. After implementation, focused suites passed `4/4` files and `50/50` tests.
- Fresh verification: full frontend suite passed `29/29` files and `202/202` tests; full backend suite passed `440/440`; `npm run build` passed with only the existing non-blocking Vite chunk-size advisory.
- Electron smoke evidence used an isolated temporary user-data directory and Chrome DevTools Protocol. Clicking `Mode Web Chat` created a second page target at `https://chatgpt.com/auth/login`; the host DOM reported the Web Chat region present, native composer/history absent, and a native viewport at `1154 x 838` pixels. The temporary smoke instance was stopped after verification.
- Scope boundary: this phase does not provide workspace access, MCP/OpenAI tunnel behavior, token extraction, cookie inspection, or website automation. Those require a separate permission/authentication phase.
- Release decision: source-only change held for a later batch. No version bump, installer build, GitHub Release, or updater publication was created.
- Skills used: `writing-plans`, `test-driven-development`, `webapp-testing`, and `verification-before-completion`.

## 2026-08-21 - Add explicit Web Chat workspace grants (Phase 2)

- Added `WebChatGrantStore`, persisted under Electron user data and deliberately separate from the `persist:web-chat` website session. It records one canonical real workspace directory, display name, requested permission profile, grant ID, timestamp, and monotonic revision; it never stores website credentials, cookies, or tokens.
- Replacing a workspace or profile first persists an empty grant, then writes the replacement. Missing, invalid, non-directory, corrupt, and stale stored paths fail closed. Explicit revoke removes the active grant without signing ChatGPT out.
- Added narrow main/preload/renderer IPC for grant state, set, revoke, and state updates. Main-process public state hard-codes `toolsEnabled: false` and `tunnelConnected: false`, so this phase grants no filesystem, MCP, shell, or tunnel capability.
- Added a compact workspace-access panel to Web Chat. It lists only projects already registered in the app, supports Manual control / Approvals only / Full access as requested profiles, shows the active scope, and provides explicit revoke. The native WebContentsView remains isolated and receives none of this IPC.
- TDD evidence: grant-store and panel tests initially failed before the implementation existed; one UI assertion then exposed an ambiguous duplicate label and was tightened to the semantic revoke control. Focused integration passed `44/44`, and IPC/relocation contracts passed `13/13`.
- Fresh verification: frontend `30/30` files and `206/206` tests passed; backend `440/440` passed; `npm run build` passed with the existing non-blocking chunk-size advisory. An isolated Electron/CDP smoke proved empty -> grant -> restart persistence -> revoke, while tools and tunnel stayed false throughout. The temporary profile was removed afterward.
- Release decision: source-only change held for a later update batch. No version bump, installer build, GitHub Release, or updater publication was created.
- Skills used: `writing-plans`, `test-driven-development`, `webapp-testing`, and `verification-before-completion`.

## 2026-08-21 - Generalize compatible API providers with presets and manual configuration

- Replaced the single free-form Custom Anthropic-compatible setup with a generic compatible-provider profile. The saved profile now records a preset ID, Base URL, API protocol, request authentication, model-list authentication, and imported model IDs; credentials remain in the existing separate local key store.
- Moved provider choices into `compatible_provider_presets.json`. MWAPI, OpenRouter, Groq, Together AI, DeepInfra, LiteLLM, and Custom are registry data rather than provider-name branches. Runtime dispatch uses only `anthropic_messages` or `openai_chat_completions` plus generic auth fields.
- Added Settings dropdowns that populate every required field from a preset while keeping Base URL, protocol, and authentication editable for custom services. Existing profiles without the new fields migrate to their previous Anthropic Messages behavior.
- Replaced Python `urllib` calls with bounded `httpx` requests for compatible model discovery and Anthropic Messages, resolving the third-party gateway's Cloudflare rejection of the prior transport. Added `httpx` as a direct runtime dependency and packaged the JSON registry.
- Live verification used the user-provided MWAPI credential transiently without saving or printing it: `/v1/models` returned seven models and `claude-haiku-4-5-20251001` returned exactly `API_OK` through the configured Messages protocol.
- Verification: backend `443/443`; frontend `30/30` files and `207/207` tests; production Vite build passed with only the existing chunk-size advisory. Focused provider tests cover registry data, legacy migration, both auth header shapes, both runtime protocols, and preset/manual UI behavior.
- Release decision: source-only batch. No version bump, installer build, GitHub Release, or updater publication was created.
- Skills used: `writing-plans`, `systematic-debugging`, `test-driven-development`, `webapp-testing`, and `verification-before-completion`.

## 2026-08-21 - Complete Web Chat Bridge Phase 3 local MCP gateway

- Added an internal `WebChatLocalMcpGateway` bound to the active grant ID, monotonic revision, permission profile, and canonical workspace root. Every dispatch revalidates that generation before delegating to the existing `WorkspaceTools` containment and Secret Guard boundary.
- Exposed only `list_directory`, `search_files`, and `read_file` from the built-in workspace catalog. Write, shell, Git, verification, restore, and unrestricted filesystem capabilities are absent. Optional provider tools are namespaced and admitted only when they explicitly declare both trusted read-only and workspace-bound annotations; malformed tool-result contracts fail closed.
- Added sidecar bind, unbind, state, and internal-call seams plus main-process synchronization across grant creation, replacement, revoke, sidecar restart, and sidecar failure. Renderer state enables local tools only when grant ID, revision, and canonical workspace path all match.
- Added metadata-only JSONL gateway auditing. Audit events retain timestamps, grant/tool identity, argument keys, status, and errors without storing tool results, file contents, credentials, cookies, or website session data.
- Updated the Web workspace panel with truthful transient local-gateway status and a compact tool catalog. It continues to show `Tunnel off` and states that tools remain local until a later authenticated tunnel phase. The remote ChatGPT `WebContentsView` still has no preload, Node integration, app IPC, or native tool access.
- TDD evidence covered row-level gateway containment, stale grants, malformed revisions, Secret Guard behavior, provider annotation/result-contract enforcement, sidecar lifecycle, renderer grant matching, and panel state. An in-session security review found and resolved workspace-path matching and malformed-generation/result fail-closed gaps.
- Verification: full backend suite passed `450/450`; full frontend suite passed `31/31` files and `209/209` tests; the production frontend build passed; an isolated Electron/CDP smoke verified grant -> three ready local tools -> panel rendering with tunnel off -> revoke/off and wrote `work_logs/web-chat-phase3-smoke.png`.
- Release decision: source-only change. No version bump, installer build, GitHub Release, updater publication, tunnel listener, or application restart was created.
- Skills used: `writing-plans`, `improve-codebase-architecture`, `test-driven-development`, `webapp-testing`, `requesting-code-review` (in-session review), and `verification-before-completion`.

## 2026-08-21 - Complete Web Chat Bridge Phase 4 authenticated tunnel

- Added `WebChatTunnelController`, a loopback-only stateless MCP JSON-RPC transport, and an injectable tunnel adapter registry. The transport requires a constant-time checked bearer credential, limits request size and MCP methods, forwards calls only through the active Phase 3 gateway generation, and never includes credentials in public state or audit metadata.
- Added the optional Cloudflare Quick Tunnel adapter. It resolves `cloudflared` from `COWORK_CLOUDFLARED_PATH` or `PATH`, parses only generated HTTPS endpoints, receives no bearer credential, uses bounded startup, reports missing-runtime errors clearly, and terminates the complete child process tree on Windows. An explicit environment-only static adapter seam supports isolated smoke testing without public network access.
- Added sidecar start/state/stop commands on a background worker. Tunnel teardown precedes gateway replacement/unbind and also runs at sidecar EOF. Idle timeout, credential expiry, adapter failure, manual disconnect, workspace revoke/change, and application quit all fail closed.
- Electron main now creates a fresh 256-bit bearer credential for each explicit start and sends it directly to the sidecar. It remains transient only in those two trusted processes; the preload exposes narrow start/stop methods and no renderer method can read the bearer. Renderer connectivity requires matching grant ID, revision, and canonical workspace path.
- Added compact Cloudflare provider, Connect/Disconnect, transient lifecycle/error, authenticated endpoint host, and expiry controls to the Web Chat workspace panel. Phase 5 connector registration remains intentionally unimplemented.
- TDD evidence: focused backend tests initially failed on the missing tunnel module, process-tree seam, and stop-during-start race; focused frontend tests initially failed on missing state normalization, bridge methods, and controls. Final focused suites passed backend `12/12` and frontend `12/12`.
- Verification: backend regression passed `462/462`; frontend regression passed `31/31` files and `211/211` tests; production build passed with the existing non-blocking chunk-size advisory. The first backend run, executed concurrently with frontend tests, exposed two timing-sensitive unrelated tests; each passed in isolation and the fresh sequential suite then passed in full. Electron/CDP smoke connected through the non-network test adapter, proved redacted state, rendered the connected controls, revoked the grant, confirmed tunnel `off`, and wrote `work_logs/web-chat-phase4-smoke.png`. `git diff --check` passed with line-ending notices only, and a credential-pattern scan found only intentional dummy keys in tests.
- Release decision: source-only Phase 4. No version bump, installer build, GitHub Release, updater publication, or Phase 5 connector setup was performed.
- Skills used: `writing-plans`, `improve-codebase-architecture`, `test-driven-development`, `systematic-debugging`, `webapp-testing`, `requesting-code-review` (in-session security review), and `verification-before-completion`.

## 2026-08-22 - Complete Web Chat Bridge Phase 5 connector setup

- Added a main-process remote MCP verifier that performs bounded authenticated `initialize` and `tools/list` calls against the exact active HTTPS tunnel before registration controls unlock. Its public result is limited to endpoint, server name, protocol version, tool count, checked time, status, and actionable error text.
- Added narrow endpoint/credential copy IPC. Electron writes directly to the OS clipboard and returns only copy metadata; the bearer never enters renderer state or logs. A copied credential is cleared after 60 seconds only when the clipboard still contains that exact value.
- Added a compact `Register in ChatGPT` checklist to the Web Chat workspace panel with transient verification status and explicit URL/credential actions. The user remains responsible for creating the custom app in ChatGPT. No DOM automation, cookie access, OAuth extraction, website preload, or automatic form filling was added.
- Connector verification is reset on tunnel stop/error, endpoint or credential generation change, grant replacement/revoke, sidecar exit, and application teardown. Post-probe state is revalidated so a stale response cannot unlock setup for a replaced tunnel. UI copy controls also require the verified endpoint to match the active endpoint.
- TDD/review found two missing acceptance proofs and added them: a stalled probe is aborted with a redacted timeout error, and verified metadata from a stale endpoint leaves both copy actions disabled. The final lifecycle review also found that the initial grant-state IPC omitted transient connector verification after leaving and re-entering Web Chat; initial hydration and live events now use one redacted `currentWebChatGrantState` projection, with a regression assertion. No blocking issue remains.
- Fresh verification: targeted Phase 5 frontend tests passed `15/15`; full backend passed `462/462`; full frontend passed `32/32` files and `217/217` tests; `npm run build` passed with only the existing non-blocking chunk-size advisory; `git diff --check` reported no whitespace errors beyond Windows line-ending notices.
- An isolated Electron/CDP smoke verified the narrow preload shape (`probe` and `copy`, no credential getter), rendered the Phase 5 registration UI, and proved revoke returns tunnel `off` with tools disabled. Evidence is `work_logs/web-chat-phase5-smoke.png`.
- Runtime limitation: `cloudflared` is not installed on this machine, so this session did not spend public network resources or claim a real ChatGPT connector registration. The source reports that missing dependency explicitly; a real public endpoint probe remains the next manual live check after installing/configuring a tunnel adapter and using a ChatGPT account that supports custom apps.
- Release decision: source-only Phase 5. No version bump, installer build, GitHub Release, updater publication, or application restart was performed.
- Skills used: `openai-docs`, `writing-plans`, `improve-codebase-architecture`, `test-driven-development`, `webapp-testing`, `requesting-code-review` (in-session review; no external reviewer invoked), and `verification-before-completion`.

## 2026-08-22 - Complete Web Chat Bridge Phase 6 approval-gated tools

- Expanded `WebChatLocalMcpGateway` from the three Phase 3 inspection tools to all ten existing bounded `WorkspaceTools` operations. `list_directory`, `search_files`, `read_file`, `list_backups`, `git_status`, and `git_diff` are annotated read-only; `write_file`, `edit_file`, `restore_backup`, and `run_verification` reuse the existing approval callbacks and permission profiles. No arbitrary shell or unrestricted code execution was exposed.
- Bound remote side effects to the exact active tunnel generation. Calls validate the generation before approval and revalidate after approval, closing the approval/disconnect race found during the in-session security review. Tunnel stop, expiry, grant replacement/revoke, gateway close, and sidecar shutdown cancel matching pending Web Chat approvals; stale or unknown decisions remain deny-by-default.
- Reused the native approval pipeline rather than adding a Web-only policy system. Approval cards include the full tool arguments, workspace/grant identity, tunnel generation, generated file diff or exact allowlisted verification command, and identify Web Chat as the origin. Manual/Trusted/Full only change prompt frequency; containment, Secret Guard, fixed verification presets, atomic writes, rollback backups, and audit behavior remain active.
- Added workspace-bound provider tool handling: explicitly annotated read-only provider tools remain direct, while ambiguous/write provider tools use the same approval path. Tools without a workspace-bound annotation are not exposed.
- The isolated ChatGPT `WebContentsView` is hidden while a native approval prompt is pending, preventing Chromium from covering the prompt, and is restored after allow/deny resolution.
- TDD evidence: focused Phase 6 backend tests passed `27/27`, including disconnect-during-approval regressions for both workspace and provider side effects. Full backend passed `470/470`; full frontend passed `32/32` files and `219/219` tests; `npm run build` passed with only the existing chunk-size advisory. The isolated Electron smoke proved the safe preload shape, tunnel connection, redacted state, and revoke -> tunnel off/tools disabled lifecycle. `git diff --check` reported no whitespace errors beyond Windows line-ending notices.
- In-session review verdict: PASS with no blocking findings after the post-approval tunnel-generation recheck was added. Credentials remain outside renderer state and logs; the website partition still receives no native preload, cookie inspection, or OAuth extraction.
- Release decision: source-only Phase 6. No version bump, installer build, GitHub Release, updater publication, or installed-app modification was performed.
- Skills used: `writing-plans`, `improve-codebase-architecture`, `test-driven-development`, `systematic-debugging`, `webapp-testing`, `requesting-code-review` (in-session review; no external reviewer invoked), and `verification-before-completion`.

## 2026-08-22 - Prepare desktop update v0.1.30

- Batched the post-`v0.1.29` compatible-provider registry/manual endpoint controls and Web Chat Bridge Phases 1 through 6 into desktop version `0.1.30` without changing either updater path.
- Fresh release verification passed: backend `470/470` when run sequentially; frontend `32/32` files and `219/219` tests; production Vite build passed with only the existing non-blocking chunk-size advisory. An initial backend run executed concurrently with Vitest missed one MCP timeout timing assertion by about 6 ms; the immediate isolated full backend run passed, so no test threshold or runtime behavior was weakened.
- `npm run dist` produced `AI-Dev-Co-worker-Setup-0.1.30.exe` (`111048513` bytes), its `117177` byte blockmap, and `latest.yml` (`364` bytes). The manifest reports version `0.1.30`, the exact installer filename and size, and SHA-512 `CNqDUuzUdwOe3H4Rz2faqMhZ+sR5vtyeCktS4rsih8qfQXTnJpEasnKcTNU/FDd9utahYgcZUjgbl9UoEg2jeA==`.
- Installer SHA-256 is `38130AFCA3212AAC34FA656199037A88FB31646C482721D74A7FA9B8A70B973A`; blockmap SHA-256 is `FF31B5BB8489BAD2A0078CF2A7AE4C8C979826039C0C1AE72B187841FBF237AF`; updater-manifest SHA-256 is `85048066400204F84C718F9C5BC2AE966C625F8C6AC422B41DDEB9CF8AEDDDBF`.
- The unpacked packaged executable reports product version `0.1.30.0`; both new Python bridge modules are present under packaged sidecar resources. `tools/release_smoke.mjs` launched the packaged executable with an isolated temporary profile and observed the renderer at the packaged `app.asar/dist/index.html`, then stopped the process tree and removed the profile.
...
## 2026-08-22 - Live-enable and verify the Web Chat Cloudflare tunnel

- Continued Web Chat Bridge validation after confirming that the embedded ChatGPT login session was healthy but no workspace grant or public connector was active. No real project workspace was granted automatically; the live smoke used an isolated temporary directory and shut the tunnel down in `finally`.
- Installed the official Cloudflare `cloudflared` runtime through WinGet (`Cloudflare.cloudflared`, version `2026.8.2`) and verified the executable version locally. This is a machine runtime dependency, not an embedded app credential or website-session shortcut.
- Root cause of the first live failure: `CloudflaredQuickTunnelAdapter` passed Node/Electron's `windowsHide` option into Python `subprocess.Popen`, which rejects that keyword before the tunnel process starts. Added a failing regression assertion first, then replaced it with Python's Windows-only `CREATE_NO_WINDOW` creation flag while preserving hidden launch, bounded startup, process-tree cleanup, expiry, and idle timeout behavior.
- The next live run opened a real Cloudflare Quick Tunnel. The authenticated public endpoint returned HTTP 200 for MCP `initialize` and `tools/list`, identified itself as `AI Dev Co-worker Web Chat Gateway`, and exposed the expected ten bounded tools: `list_directory`, `search_files`, `read_file`, `write_file`, `edit_file`, `list_backups`, `restore_backup`, `git_status`, `git_diff`, and `run_verification`. The temporary tunnel and process tree were stopped after the smoke test; no bearer credential or temporary public URL was logged here.
- Added bounded connector-probe retry for transient thrown network/DNS errors that can occur while a new `trycloudflare.com` hostname becomes reachable. Authentication rejection and valid HTTP/MCP errors are not retried, and the existing overall abort timeout remains the hard upper bound.
- Manual security-focused diff review found no blocking issue: the retry path cannot expose the bearer through public state, does not retry authentication or protocol failures, and does not alter workspace grants, tool policy, side-effect approvals, or tunnel-generation revalidation.
- Fresh verification: backend `471/471`; frontend `33/33` files and `229/229` tests; production Vite build passed with only the existing non-blocking chunk-size advisory. No desktop version bump, installer build, GitHub release, or installed-app modification was performed in this source-development task.
- Skills used: `writing-plans`, `systematic-debugging`, `test-driven-development`, `requesting-code-review` (manual review because no subagent tool is available), and `verification-before-completion`.

## 2026-08-23 - Prepare Web Chat usability update v0.1.33

- Added a one-action `Connect & verify` Web Chat workspace flow. A user click starts the existing bounded tunnel and, after the backend reports that tunnel generation connected, performs exactly one connector probe automatically. Disconnect/failure clears the pending auto-probe marker; restart does not silently probe or reconnect.
- Preserved all security and delivery boundaries: registration in ChatGPT remains a deliberate user action, credentials are not exposed to renderer state or auto-copied, authentication/protocol failures are not retried, and both the startup updater and in-app top-right Update control remain unchanged.
- Live tunnel validation used an isolated temporary workspace and the installed official `cloudflared 2026.8.2`. Public MCP `initialize` and `tools/list` returned HTTP 200 with the expected ten bounded workspace tools; the temporary tunnel and process tree were stopped afterward.
- Fresh verification passed: backend `471/471`; frontend `33/33` files and `229/229` tests; production build; packaged release smoke at `v0.1.33`; and browser UI inspection of Chat/Web navigation, the workspace-access panel, and an empty console-error log.
...
## 2026-09-11 - Published code-block copy update v0.1.41

- Published source commit `2bc8737` and annotated tag `v0.1.41` to `origin/main` and GitHub Releases.
- The public release is neither a draft nor a prerelease. Assets downloaded back from GitHub exactly matched the locally verified sizes and SHA-256 digests for the installer, blockmap, and updater manifest.
- The public updater manifest returned version `0.1.41`, installer `AI-Dev-Co-worker-Setup-0.1.41.exe`, and size `116105714` with exact local manifest SHA-256 `54DA6F08E62589691C922BDC9F708E8AD3F7F33D0E0B1BA7198960DFEE5DBAF8`.
...
## 2026-09-11 - Published code-block TXT download v0.1.43

- Published source commit `9ca1809` and annotated tag `v0.1.43` to `origin/main` and GitHub Releases.
...
- The public updater manifest returned version `0.1.43`, installer `AI-Dev-Co-worker-Setup-0.1.43.exe`, and size `116105860` with exact manifest SHA-256 `B3A667F9B4598DF8490A1F6089B95C48F50607655E1C8B4919323D0540005AF9`.
- Both established delivery paths remain active: the startup update gate and the in-app top-right Update control can discover and install `v0.1.43`.
- Release URL: `https://github.com/nattankon/AI_DEV_COWORKER/releases/tag/v0.1.43`.
- Skills used: `verification-before-completion`.

## 2026-09-11 - Add adaptive native Chat context recovery

- Traced frequent long-conversation failures to the native Chat planner trusting the provider catalog's advertised context window too closely. The reproduced Z.ai response was HTTP 400/code 1261 with `Prompt exceeds max length`; opaque HTTP 5xx provider failures can represent the same condition.
- Added a configurable 65% soft context target that compacts older complete dialogue units before the hard provider limit while preserving all system/Role messages, the current user request, and the most recent complete turns. Rolling summary state now reuses the prior summary and summarizes only the newly compacted prefix instead of repeatedly processing the full old conversation.
- Added one same-model recovery attempt at a configurable 45% target for explicit context-limit errors, including Z.ai code 1261, and bounded HTTP 5xx failures. Any partial stream is reset before retry; if the retry still fails, the existing model fallback chain continues unchanged.
- Added a sanitized `chat_context` IPC diagnostic containing only session/model and token/count metadata. Native Chat's context indicator now prefers this Backend-planned estimate and reports compacted-message count, while retaining the local estimate before the first diagnostic.
- Safety and rollback: Chat still receives no workspace-write or shell capability, system/Role messages remain mandatory, the retry is capped at one attempt per candidate, and the change can be rolled back independently through the planner target, retry classifier, and `chat_context` event without altering persisted conversation storage.
- TDD evidence included expected red failures for the missing soft target, retry reducer, code-1261 classification, rolling-summary reuse, IPC allowlist, bridge subscription, and Backend indicator precedence. Fresh verification passed: focused backend `115/115`; focused frontend `73/73`; complete backend `487/487`; complete frontend `33/33` files and `244/244` tests; and production Vite build. Playwright against the live Vite UI confirmed `25%` Backend-planned usage and a tooltip reporting eight compacted messages with no overlap. The existing Python MCP deprecation and Vite chunk-size advisories remain non-blocking.
- Reviewed the scoped working tree and left unrelated untracked `TUNER.txt` untouched. This is a post-`v0.1.43` source change; no installer or updater release was produced in this task.
- Skills used: `writing-plans`, `systematic-debugging`, `test-driven-development`, `webapp-testing`, and `verification-before-completion`.

## 2026-09-11 - Replace `/compact` hard trim with durable semantic compaction

- Root cause: the existing React slash-command handler only replaced the current session timeline with its last eight arbitrary events. It never called the backend, created a semantic summary, or preserved that summary as model context, so important details disappeared even though the command appeared to work.
- Added a dedicated Chat-only `compact_chat` request and `chat_compaction` result across Electron, preload, renderer bridge, and Python sidecar. The selected model summarizes older complete dialogue units, merges any previous marked summary, preserves the four most recent complete turns, updates sidecar history, and reports original/compacted/retained counts. Histories too short to compact avoid a model call and return a bounded no-op result.
- React replaces the old dialogue with a hidden marked system-summary event plus the retained original message events, preserving recent attachment/timestamp presentation. The summary persists through the existing dual session store, is restored after restart, and is sent with future Chat requests while remaining hidden from the human timeline. A neutral green notice reports the result; `/compact` is never sent as a normal model prompt.
- Safety and rollback: only system history beginning with the exact compact-summary marker is accepted from renderer history; arbitrary system entries are rejected. Global Role remains a separate system layer from the Role store, and Chat gains no workspace, shell, or write permissions. The feature can be rolled back at the dedicated IPC command/result boundary without altering ordinary Chat generation or persisted session schema.
- TDD evidence included expected red failures for the missing history splitter, unknown backend command, dropped prior summary, missing bridge method, missing IPC allowlist entry, old eight-event trim, hidden-summary persistence, token double-counting, unmarked system-history acceptance, and success-notice styling.
- Fresh verification passed: focused backend `112/112`; focused frontend `71/71`; complete backend `492/492`; complete frontend `33/33` files and `246/246` tests; production Vite build; and Node syntax checks for Electron main/preload. Playwright against the live Vite UI confirmed that older messages disappear, four recent turns remain, the summary stays hidden, and the completion notice renders in the neutral success style without overlap. The existing Python MCP deprecation and Vite chunk-size advisories remain non-blocking.
- Reviewed the scoped working tree and left unrelated untracked `TUNER.txt` untouched. This remains post-`v0.1.43` source work; no installer or updater release was produced.
- Skills used: `writing-plans`, `systematic-debugging`, `test-driven-development`, `webapp-testing`, and `verification-before-completion`.

## 2026-09-12 - Prepare adaptive Chat context update v0.1.44

- Combined adaptive native Chat context recovery and durable semantic `/compact` into desktop release candidate `0.1.44`. The release preserves Chat/Cowork/Code permission boundaries and leaves unrelated untracked `TUNER.txt` outside the package and commit.
