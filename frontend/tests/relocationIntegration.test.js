...
describe("standalone project independence", () => {
  ...
  it("packages the official OpenAI tunnel runtime and its notices", () => {
    const packageJson = JSON.parse(fs.readFileSync(path.join(projectRoot, "package.json"), "utf8"));
    const coworkResource = packageJson.build.extraResources.find(
      (resource) => resource.to === "cowork-sidecar/AI_DEV_COWORKER",
    );

    expect(coworkResource?.filter).toContain("tools/tunnel-client/**");
    expect(
      fs.existsSync(
        path.join(projectRoot, "tools", "tunnel-client", "windows-amd64", "tunnel-client-runtime.exe"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(path.join(projectRoot, "tools", "tunnel-client", "windows-amd64", "LICENSE")),
    ).toBe(true);
  });
  ...
  it("exposes only narrow workspace IPC methods", () => {
    ...
    expect(preloadSource).toMatch(/getWebChatGrantState:\s*\(\)/);
    expect(preloadSource).toMatch(/setWebChatGrant:\s*\(payload\)/);
    expect(preloadSource).toMatch(/revokeWebChatGrant:\s*\(\)/);
    expect(preloadSource).toMatch(/startWebChatTunnel:\s*\(payload\)/);
    expect(preloadSource).toMatch(/probeWebChatConnector:\s*\(\)/);
    ...
    expect(mainSource).toMatch(/ipcMain\.handle\("web-chat-grant-state"/);
    expect(mainSource).toMatch(/ipcMain\.handle\("web-chat-grant-set"/);
    expect(mainSource).toMatch(/ipcMain\.handle\("web-chat-grant-revoke"/);
    expect(mainSource).toMatch(/ipcMain\.handle\("web-chat-tunnel-start"/);
    ...
    expect(mainSource).toMatch(/taskkill[\s\S]*"\/T"[\s\S]*"\/F"/);
